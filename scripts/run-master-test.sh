...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  echo "Usage: ./scripts/run-master-test.sh [--allow-live] [target_url] [bench_dir]"
  echo "Default target_url: http://127.0.0.1:8000/bitaxe-oc/"
  echo "Default bench_dir : <project>/bench"
...
START_TS="$(date +%s)"
echo "[info] MASTER TEST START"
echo "[info] target=${TARGET_URL}"
echo "[info] bench=${BENCH_DIR}"

echo "[info] Phase 0/4: Backend Unit + Smoke + Analyzer fixtures"
php "${ROOT_DIR}/scripts/backend-unit.php"
php "${ROOT_DIR}/scripts/backend-smoke.php"
php "${ROOT_DIR}/scripts/analyzer-fixtures.php"

echo "[info] Phase 1/4: HTTP Audit (SEO + performance + security + API)"
node "${ROOT_DIR}/scripts/live-http-audit.js" "${TARGET_URL}"

echo "[info] Phase 1.5/4: Ops Panel Audit (auth/security/runtime)"
node "${ROOT_DIR}/scripts/ops-panel-audit.js" "${TARGET_URL}"

echo "[info] Phase 2/4: Live E2E (functional + security + share API DB path)"
SAFARI_WEBDRIVER_URL="${DRIVER_URL}" node "${ROOT_DIR}/scripts/live-e2e-safari.js" "${TARGET_URL}"

echo "[info] Phase 3/4: Live Bench (limit/load scenarios)"
SAFARI_WEBDRIVER_URL="${DRIVER_URL}" node "${ROOT_DIR}/scripts/live-bench-safari.js" "${TARGET_URL}" "${BENCH_DIR}"

END_TS="$(date +%s)"
TOTAL_SEC=$((END_TS - START_TS))
echo "[info] MASTER TEST DONE (${TOTAL_SEC}s)"
